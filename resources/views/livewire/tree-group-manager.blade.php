            ...
            <div class="card shadow-sm" style="border-radius: 16px; background: #f5f7fa;">
                <div class="card-body">
                    <h4 class="mb-4" style="color: #1976d2; font-weight: bold;">
                        ...
                        @if($editId) ویرایش گروه @else افزودن گروه جدید @endif
                    </h4>
                    <form wire:submit.prevent="{{ $editId ? 'update' : 'save' }}" autocomplete="off">
                        <div class="form-row">
                            <div class="form-group col-md-8 mb-3">
                                <label class="mb-1" for="name" style="color: #1976d2;">نام گروه</label>
                                @if($editId)
            ...
            <div class="card mt-4 shadow-sm" style="border-radius: 16px; background: #e3f2fd;">
                <div class="card-body">
                    <h5 class="mb-3" style="color: #1976d2; font-weight: bold;">
                        <i class="material-icons align-middle">list</i> لیست گروه‌ها
                    </h5>
                            ...
                            <tbody>
                                @forelse($groups as $group)
                                    <tr>
                                        <td>{{ $group->name }}</td>
                                        <td>
                                            <button wire:click="edit({{ $group->id }})" class="btn btn-sm" style="background: #ffd600; color: #333; border-radius: 6px;">
                                                <i class="material-icons align-middle">edit</i>
                                            </button>
                                            <button wire:click="delete({{ $group->id }})" class="btn btn-sm" style="background: #e53935; color: #fff; border-radius: 6px;" onclick="return confirm('حذف شود؟')">
                                                <i class="material-icons align-middle">delete</i>
                                            </button>
                                        </td>
                                    </tr>
                                @empty
                                    <tr><td colspan="2" class="text-center">گروهی وجود ندارد.</td></tr>
                                @endforelse
                            </tbody>
                        </table>
                    </div>
