                    ...
                    <form action="{{ route('planted-trees.update', $plantedTree->id) }}" method="POST" enctype="multipart/form-data" autocomplete="off">
                        ...
                        <div class="form-row">
                            <div class="form-group col-md-4 mb-3">
                                ...
                                <select name="tree_group_id" id="tree_group_id" class="form-control" style="border-radius: 8px; background: #fff;" required>
                                    ...
                                    @foreach($treeGroups as $group)
                                        <option value="{{ $group->id }}" {{ $plantedTree->tree_group_id == $group->id ? 'selected' : '' }}>{{ $group->name }}</option>
                                    @endforeach
                                </select>
                            ...
                            <div class="form-group col-md-4 mb-3">
                                ...
                                <select name="tree_id" id="tree_id" class="form-control" style="border-radius: 8px; background: #fff;" required>
                                    ...
                                    @foreach($trees as $tree)
                                        <option value="{{ $tree->id }}" {{ $plantedTree->tree_id == $tree->id ? 'selected' : '' }}>{{ $tree->name }}</option>
                                    @endforeach
                                </select>
                            ...
                            <div class="form-group col-md-4 mb-3">
                                ...
                                <select name="status" id="status" class="form-control" style="border-radius: 8px; background: #fff;" required>
                                    <option value="سالم" {{ $plantedTree->status == 'سالم' ? 'selected' : '' }}>سالم</option>
                                    <option value="بیمار" {{ $plantedTree->status == 'بیمار' ? 'selected' : '' }}>بیمار</option>
                                    <option value="خشک شده" {{ $plantedTree->status == 'خشک شده' ? 'selected' : '' }}>خشک شده</option>
                                </select>
                            ...
                            <div class="form-group col-md-6 mb-3">
                                ...
                                <select name="location_id" id="location_id" class="form-control" style="border-radius: 8px; background: #fff;" required>
                                    ...
                                    @foreach($locations as $location)
                                        <option value="{{ $location->id }}" {{ $plantedTree->location_id == $location->id ? 'selected' : '' }}>{{ $location->name }}</option>
                                    @endforeach
                                </select>
                            ...
                            <div class="form-group col-md-6 mb-3">
                                ...
                                <select name="position_id" id="position_id" class="form-control" style="border-radius: 8px; background: #fff;" required>
                                    ...
                                    @foreach($positions as $position)
                                        <option value="{{ $position->id }}" {{ $plantedTree->position_id == $position->id ? 'selected' : '' }}>{{ $position->name }}</option>
                                    @endforeach
                                </select>
                        ...
                        <div class="form-row">
                            ...
                            <div class="form-group col-md-3 mb-3">
                                ...
                                <input type="number" step="0.000001" name="latitude" id="latitude" class="form-control" style="border-radius: 8px; background: #fff;" value="{{ old('latitude', $plantedTree->latitude) }}" required>
                                @error('latitude')<span class="text-danger">{{ $message }}</span>@enderror
                            </div>
                            <div class="form-group col-md-3 mb-3">
                                <label class="mb-1" for="longitude" style="color: #1976d2;">عرض جغرافیایی</label>
                                <input type="number" step="0.000001" name="longitude" id="longitude" class="form-control" style="border-radius: 8px; background: #fff;" value="{{ old('longitude', $plantedTree->longitude) }}" required>
                        ...
                        <div class="form-row">
                            ...
                            <div class="form-group col-md-6 mb-3">
                                ...
                                <textarea name="description" id="description" class="form-control" style="border-radius: 8px; background: #fff;" rows="3">{{ old('description', $plantedTree->description) }}</textarea>
